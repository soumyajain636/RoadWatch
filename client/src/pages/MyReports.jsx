...
export default function MyReports() {
  ...
  useEffect(() => {
    ...
    const loadReports = async () => {
      try {
        ...
        const { data } = await reportService.getMine(params)
        ...
        handleApiError(error, 'Could not load your reports.')
      } finally {
        if (!ignore) {
          setIsLoading(false)
        }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setReports((prev) =>
        prev.filter((r) => r._id !== targetReport._id)
      )
  ...
  return (
    <PageTransition title="My Reports">
        ...
        <div className="mt-6">
          ...
          {isLoading ? (
            ...
            <EmptyReports
              ...
              message="Try clearing a filter or submit your first report."
              showCreateAction
            />
          ) : view === 'table' ? (
            <ReportTable
